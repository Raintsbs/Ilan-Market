...
function Shader({ source, uniforms }: ShaderProps) {
  return (
    <Canvas className="absolute inset-0 h-full w-full" frameloop="always" dpr={[1, 1.5]}>
      <ShaderMaterial source={source} uniforms={uniforms} />
    </Canvas>
  );
}

const BRAND_COLORS: number[][] = [
  [59, 130, 246],
  [37, 99, 235],
];

export function AuthFlowBackground({ showSuccessReveal = false }: { showSuccessReveal?: boolean }) {
  ...
  React.useEffect(() => {
    if (!showSuccessReveal) return;
    setReverseCanvasVisible(true);
    const timer = window.setTimeout(() => setInitialCanvasVisible(false), 50);
